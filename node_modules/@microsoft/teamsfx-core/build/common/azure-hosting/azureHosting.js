...
class AzureHosting {
    ...
    async generateBicep(bicepContext, pluginId) {
        var _a, _b;
        // * The order matters.
        ...
        const modules = await Promise.all(bicepFiles.map(async (filename) => {
            const module = await __1.generateBicepFromFile(path_1.default.join(bicepTemplateDir, filename), bicepContext);
    ...
    async updateBicep(bicepContext, pluginId) {
        ...
        const bicepTemplateDir = this.getBicepTemplateFolder();
        let module = await __1.generateBicepFromFile(path_1.default.join(bicepTemplateDir, bicepFile), bicepContext);
