...
async function deploy(ctx, inputs, envInfo, tokenProvider) {
    ...
    const botTroubleShootMsg = utils_1.getBotTroubleShootMessage(utils_1.getAzureSolutionSettings(ctx));
    ...
    if (result.kind === "success") {
        if (inAzureProject) {
            let msg = localizeUtils_1.getLocalizedString("core.deploy.successNotice", ctx.projectSetting.appName) +
            ...
            if (botTroubleShootMsg.textForLogging && !isDeployAADManifestFromVSCode) {
                ...
                ctx.userInteraction
                    .showMessage("info", `${localizeUtils_1.getLocalizedString("core.deploy.successNotice", ctx.projectSetting.appName)} ${botTroubleShootMsg.textForMsgBox}`, false, botTroubleShootMsg.textForActionButton)
                    .then((result) => {
