...
async function getQuestionsForResourceGroup(defaultResourceGroupName, existingResourceGroupNameLocations, availableLocations) {
...
async function askResourceGroupInfo(ctx, azureAccountProvider, rmClient, inputs, ui, defaultResourceGroupName) {
    ...
    if (!common_1.isMultiEnvEnabled()) {
        ...
        try {
            ...
            (_a = ctx.logProvider) === null || _a === void 0 ? void 0 : _a.warning(`Failed to check resource group existence ${defaultResourceGroupName}, assume non-existent, error '${error}'`);
        }
    ...
    try {
        ...
        (_b = ctx.logProvider) === null || _b === void 0 ? void 0 : _b.error(`Failed to list resource group: error '${error}'`);
    ...
    const node = await getQuestionsForResourceGroup(defaultResourceGroupName, resourceGroupNameLocations, locations.value);
    if (node) {
        const res = await teamsfx_api_1.traverse(node, inputs, ui);
        if (res.isErr()) {
            (_c = ctx.logProvider) === null || _c === void 0 ? void 0 : _c.debug(`[${constants_2.PluginDisplayName.Solution}] failed to run question model for target resource group.`);
            return teamsfx_api_1.err(res.error);
        }
        const desensitized = questionModel_1.desensitize(node, inputs);
        (_d = ctx.logProvider) === null || _d === void 0 ? void 0 : _d.info(`[${constants_2.PluginDisplayName.Solution}] success to run question model for resource group, answers:${JSON.stringify(desensitized)}`);
    }
...
async function askCommonQuestions(ctx, appName, config, azureAccountProvider, appstudioTokenJson) {
    ...
    const defaultResourceGroupName = `${appName.replace(" ", "_")}${common_1.isMultiEnvEnabled() ? "-" + ctx.envInfo.envName : ""}-rg`;
