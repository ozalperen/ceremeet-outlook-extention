...
class Factory {
    ...
    static async buildAadManager(graphTokenProvider, telemetryReporter, logProvider) {
        const lazyAadService = new commonUtils_1.Lazy(async () => await Factory.buildAadService(graphTokenProvider, telemetryReporter, logProvider));
        return new aadManager_1.AadManager(lazyAadService, telemetryReporter, logProvider);
    }
    static async buildTeamsAppAadManager(graphTokenProvider, telemetryReporter, logProvider) {
        const lazyAadService = new commonUtils_1.Lazy(async () => await Factory.buildAadService(graphTokenProvider, telemetryReporter, logProvider));
