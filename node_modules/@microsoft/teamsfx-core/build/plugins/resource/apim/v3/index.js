...
let ApimPluginV3 = class ApimPluginV3 {
    ...
    async getQuestionsForDeploy(ctx, inputs, envInfo, tokenProvider) {
        const apimConfig = new config_1.ApimPluginConfig(envInfo.state[this.name], envInfo.envName);
    ...
    async provisionResource(ctx, inputs, envInfo, tokenProvider) {
        ...
        const apimState = envInfo.state[this.name];
        ...
        const aadManager = await factory_1.Factory.buildAadManager(tokenProvider.graphTokenProvider, ctx.telemetryReporter, ctx.logProvider);
    ...
    async configureResource(ctx, inputs, envInfo, tokenProvider) {
        ...
        const aadManager = await factory_1.Factory.buildAadManager(tokenProvider.graphTokenProvider, ctx.telemetryReporter, ctx.logProvider);
        const teamsAppAadManager = await factory_1.Factory.buildTeamsAppAadManager(tokenProvider.graphTokenProvider, ctx.telemetryReporter, ctx.logProvider);
