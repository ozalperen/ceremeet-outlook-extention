...
let TeamsBot = TeamsBot_1 = class TeamsBot {
    ...
    async localDebug(context) {
        logger_1.Logger.setLogger(context.logProvider);
        const result = await errors_1.runWithExceptionCatching(context, () => this.getImpl(context).localDebug(context), false, constants_1.LifecycleFuncNames.LOCAL_DEBUG);
    ...
    async postLocalDebug(context) {
        logger_1.Logger.setLogger(context.logProvider);
        return await errors_1.runWithExceptionCatching(context, () => this.getImpl(context).postLocalDebug(context), false, constants_1.LifecycleFuncNames.POST_LOCAL_DEBUG);
    }
    async getQuestions(stage, context) {
        ...
        if (stage === teamsfx_api_1.Stage.create) {
            return await errors_1.runWithExceptionCatching(context, async () => {
                var _a, _b;
                if (common_1.isVSProject(context.projectSettings) || ((_a = context.answers) === null || _a === void 0 ? void 0 : _a.platform) === teamsfx_api_1.Platform.VS) {
                    const res = new teamsfx_api_1.QTreeNode(question_2.createHostTypeTriggerQuestionForVS());
                    res.condition = question_2.showNotificationTriggerCondition;
                    return teamsfx_api_1.ok(res);
                }
                else if (common_1.isBotNotificationEnabled()) {
                    const res = new teamsfx_api_1.QTreeNode({
                        type: "group",
                    });
                    res.addChild(new teamsfx_api_1.QTreeNode(question_2.createHostTypeTriggerQuestion((_b = context.answers) === null || _b === void 0 ? void 0 : _b.platform)));
                    res.condition = question_2.showNotificationTriggerCondition;
                    return teamsfx_api_1.ok(res);
                }
                else {
