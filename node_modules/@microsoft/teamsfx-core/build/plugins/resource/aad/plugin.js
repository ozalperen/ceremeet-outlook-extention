    ...
    async provisionUsingManifest(ctx, isLocalDebug = false) {
        ...
        if (manifest.id) {
            ...
            if (!skip || !existingOauth2PermissionScopeId) {
                ...
                config = await aadAppClient_1.AadAppClient.getAadAppUsingManifest(telemetryMessage, manifest.id, config.password, await this.getScopeIdForTeams(manifest), ctx.graphTokenProvider, ctx.envInfo.envName);
