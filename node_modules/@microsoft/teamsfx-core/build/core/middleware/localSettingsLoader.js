...
const LocalSettingsLoaderMW = async (ctx, next) => {
    ...
    if (!projectSettingsLoader_1.shouldIgnored(ctx) && !tools_1.isConfigUnifyEnabled()) {
        ...
        if (solutionSettings) {
            ...
            if (exists) {
                ctx.localSettings = await localSettingsProvider.loadV2((_a = ctx.contextV2) === null || _a === void 0 ? void 0 : _a.cryptoProvider, hasAAD);
            }
            else {
                ctx.localSettings = localSettingsProvider.initV2(hasFrontend, hasBackend, hasBot, hasSimpleAuth, hasAAD);
            }
            if (ctx.solutionContext) {
                if (exists) {
                    ...
                    ctx.solutionContext.localSettings = localSettingsProvider.init(hasFrontend, hasBackend, hasBot, hasSimpleAuth, hasAAD);
                }
            }
        }
    }
